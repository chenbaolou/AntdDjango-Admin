...
def _get_attr(model_data, time_func, foreign, many):
	...
	for itm in attr_list:
		attribute = getattr(model_data, itm, None)
		if attribute == None:
			continue
		if isinstance(attribute, models.Model):
			...
			dic_list[itm] = time_func(attribute)
		elif isinstance(attribute, FieldFile):
			dic_list[itm+"name"] = os.path.basename(str(attribute))
			dic_list[itm+"path"] = os.path.dirname(str(attribute))
		else:
			dic_list[itm] = attribute
...
def serializer(data, datetime_format='timestamp', output_type='raw', include_attr=None, exclude_attr=None, foreign=False, many=False, **kwargs):
